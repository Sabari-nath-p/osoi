...
import 'package:oso/screens/cart/coupon.dart';
import 'package:oso/screens/home/components/cartproductview.dart';
import 'package:quickalert/quickalert.dart';
import 'package:oso/Converter/placeOrder.dart';
...
class cart extends StatefulWidget {
  ...
  @override
  State<cart> createState() => _cartState(charges: charges);
}

class _cartState extends State<cart> {
  var charges;
  _cartState({required this.charges});
  List orderOption = [];
  List paymentMethod = [
    ["Stripe", "STRIPE"],
  ...
  double deliveryCharge =
      0; //double.parse(widget.charges["shipping_charge"]).toDouble();
  int walletPoint = 0;
  List pdata = [];
      ...
      if (checkOutdata["unavailable_products"].length > 0) {
        ...
        setState(() {
          for (var unproduct in checkOutdata["unavailable_products"]) {
            ...
            pd = product.get(int.parse(unproduct));
            pd.quantity = 0;
    ...
    setState(() {
      for (var p in box.keys) {
        ...
        if (pd.quantity > 0) {
          ...
          itemdiscount = itemdiscount + pd.save;
  ...
  String selectedShop = "0";
  int selectedCharity = 0;
  List Charity = [];

  loadShop() async {
    print("loadShop");
    SharedPreferences pref = await SharedPreferences.getInstance();
    selectedShop = pref.getString("SHOP_ID").toString();
    final Response =
        await http.get(Uri.parse("$baseurl/shops?id=$selectedShop"));
    print(Response.statusCode);
    if (Response.statusCode == 200) {
      var cdata = json.decode(Response.body);
      print(cdata["data"][0]["charity_organisations"]);
      if (cdata["data"] != null)
        for (var vm in cdata["data"][0]["charity_organisations"]) {
          print("working 2 ");
          setState(() {
            Charity.add(vm);
          });
  ...
  void initState() {
    ...
    print("123456");
    deliveryCharge =
        double.parse(charges["shipping_charge"].toStringAsFixed(2));
    print(charges["shipping_charge"]);
    totalTax = double.parse(charges["total_tax"].toStringAsFixed(2));
    print(totalTax);

    walletPoint = double.parse(charges["wallet_amount"].toString()).round();
    awalletPoint = walletPoint;
    pickupDiscount =
        double.parse(charges["pickup_discount_amount"].toStringAsFixed(2))
            .roundToDouble();
    if (charges["wallet_currency"] > 0)
      currencyRatio = (walletPoint /
              double.parse(
                      charges["wallet_currency"].toStringAsFixed(2).toString())
                  .toDouble())
          .round();
    print("1234567");
  }
  ...
  Widget build(BuildContext context) {
    ...
    double walletCurrency = (walletPoint / currencyRatio);
    subtotal = itemTotal +
        deliveryCharge +
        totalTax -
        pickupDiscount -
    ...
    if (addWalletAmount) {
      ...
      if (walletCurrency > subtotal) {
        ...
        walletPoint = (walletCurrency * currencyRatio).round();
      } else {
        walletPoint = 0;
        walletCurrency = 0;
      }
    ...
    walletCurrency = walletCurrency.roundToDouble();
                          ...
                          child: Row(
                            children: [
                              ...
                              Container(
                                  ...
                                  alignment: Alignment.center,
                                  child: Image.network(
                                      "https://cdn1.iconfinder.com/data/icons/ecommerce-line-circle/605/64_-_Discount-512.png")),
                              width(density(19)),
                              ...
                              if (couponData.length > 0)
                                if (couponData[1] != 0)
                                  paymentBox(
                                      ...
                                      Colors.black),
                              if (pickupDiscount != 0)
                                paymentBox("Pickup Discount",
                                    "\$ ${pickupDiscount}", Colors.black),
                              if (itemdiscount != 0)
                                paymentBox("Discount", "\$ ${itemdiscount}",
                                    Colors.black),
                              if (discount != 0)
                                paymentBox("Discount", "\$ ${discount}",
                                    Colors.black.withOpacity(.8)),
                              if (totalTax != 0)
                                paymentBox("Total Tax", "\$ ${totalTax}",
                                    Colors.black.withOpacity(.8)),
                              paymentBox(
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                "OSOI will donate 2% from total amount to the following selected Organization",
                                style: TextStyle(
                                    fontSize: 10,
                                    fontFamily: "Monsterrats",
  ...
  void place() async {
    placeAddress billingAddress = placeAddress(
      city: addressList[selectedAddress].city,
      country: addressList[selectedAddress].country,
      state: addressList[selectedAddress].state,
      zip: addressList[selectedAddress].zip,
    ...
    porder.discount = discount; //int.parse(couponData[1]).to;
    // }
    porder.amount = itemTotal;
    porder.total = itemTotal + deliveryCharge + totalTax - porder.discount!;
    porder.paidTotal = (total * 100).round(); // total - wallet point
    porder.deliveryType = widget.other["delivery_type"];
    porder.orderType =
        (widget.other["order_type"] == "pre_order") ? "pre_order" : "one_time";

    if ((widget.other["order_type"] == "pre_order"))
      porder.preOrderDate = widget.other["pickup_date"];

    print("pickup date");
    print(widget.other["products"]);
    print(widget.other["pickup_date"]);
    ...
    porder.products = widget.other["products"];

    porder.useWalletPoints = addWalletAmount;

    porder.paymentGateway = "STRIPE"; //paymentMethod[paymentMethodSelector][1];
    ...
    if (total != 0) {
      ...
      if (Response.statusCode == 200 || Response.statusCode == 201) {
        ...
        if (generatedId["error"] == null) {
          porder.paymentIntent = generatedId["payment_intent"].toString();
          print(Response.body);
  ...
  loadOnlinePayment(PlaceOrder porder, String clientID) async {
    ...
    try {
      ...
      print(e);
      print("error due to payment failed");
  ...
  verifyRazorpay() async {
    final response = await http.post(Uri.parse("$baseurl/api/place-order"),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token',
          'content-type': 'application/json',
        },
        body: jsonEncode({"payment_intent": porder.paymentIntent}));
